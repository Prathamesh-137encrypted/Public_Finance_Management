...
- **Type**: Permissioned blockchain (**Mandatory: Hyperledger Fabric v2.5+**).
- **Channel**: `fiscal-channel`.
- **MSP**: TaxAuthority, Treasury, Auditor orgs.
- **Chaincode lifecycle**: v2.0 (package/approve/commit).
...
### Phase 1
- Fabric network + CA + channel setup (docker-compose)

### Phase 2
- TaxCalculationChaincode + InvoiceValidationChaincode + unit tests

### Phase 3
- Backend API + event indexer

### Phase 4
- Stub React dashboards + demo scripts
...
1. docker-compose.yml + crypto-config + Fabric network
2. Go chaincode (tax/validate/refund) + lifecycle scripts
3. Node.js API (/submitInvoice, /validate, /refund)
4. Postgres indexer + basic React taxpayer dashboard
5. Postman collection for workflows A-E
6. README + make dev-up script + demo data
